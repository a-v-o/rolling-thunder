...
export async function pollWallet(chatId, walletAddress, chain) {
  ...
  if (lastTimestamp) {
    params.after = lastTimestamp.toISOString();
  }
  ...
  const uniqueEvents = allEvents.filter((event) => {
    if (!event.collectionSlug || seen.has(event.collectionSlug)) return false;
    seen.add(event.collectionSlug);
    return true;
  });
  ...
  for (const event of uniqueEvents) {
    if (await isFreeMint(event.collectionSlug)) {
  ...
  if (freeMints.length > 0) {
    const latestTimestamp = freeMints
      .map((e) => new Date(e.eventTimestamp))
...
export async function pollAllTrackedWallets(chatId) {
  ...
  for (const result of results) {
    if (result.status === "fulfilled") {
      ...
      console.error(
        `Poll failed for wallet: ${result.polledWallet} ${result.reason?.message}`,
      );
    }
  }
